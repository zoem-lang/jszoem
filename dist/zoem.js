...
class Tokenizer {
  ...
  handleStringLiterals() {
    ...
    if (this.peek() === '') {
      throw new ZoemError('Unfinished string', this.startPosition, this.endPosition);
    }
...
class Parser {
  ...
  function() {
    ...
    return new ZoemFunction(name, params, body);
  }
...
class Interpreter {
  ...
  constructor(environment, source) {
    ...
    const nativePrint = new ZoemCallable(null, this.env);
  ...
  evaluate(expr) {
    ...
    else if (expr instanceof ZoemFunction) return this.visitFunction(expr);
  ...
  visitFunction(expr) {
    const fn = new ZoemCallable(expr, this.environment);
...
// export { Environment, Interpreter, Parser, Tokenizer, formatZoemError, parse, run };